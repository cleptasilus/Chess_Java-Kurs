...
public class RookMovement {
	...
	public boolean CheckRook2(CurrentBoard board, Square start, Square target,
		...
		for (int n = 0; n < 8; n++) {
			...
			if (!(movedpiecestart.OccupiyingPiece.getName() == "K")) {
				if (((target.getPositionx() == start.getPositionx() + n || target
					...
					// Prüft ob der Weg frei ist und er schlagen darf
					// Return false wenn weg nicht frei oder Zielfeld eigene
					// Figur
					// Return true wenn Zug legal
					...
					.getPositiony()))
					|| ((movedpiecetarget.getPositiony() == start
							.getPositiony() + n || movedpiecetarget
	...
	public boolean CheckRook3(CurrentBoard board, Square start, Square target,
