...
public class CentralMovement {
	public boolean piecemoveslikethat(CurrentBoard board, Square start,
			Square target) {
		if(!(start== board.Squares[0][0])){		
		...
		case "":
			...
			if (!((target.getPositiony() - start.getPositiony() == 2 || target
					.getPositiony() - start.getPositiony() == -2) && target
					.getPositionx() - start.getPositionx() == 0)) {
	...
	public boolean piecemoveslikethat2(CurrentBoard board, Square start,
			Square target) {
	if(!(start== board.Squares[0][0])){	
		switch (start.OccupiyingPiece.getName()) {
		case "K":
