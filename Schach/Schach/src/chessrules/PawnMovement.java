...
public class PawnMovement {
	...
	public boolean MovePawn(CurrentBoard board, Square start, Square target) {
		if ((start.OccupiyingPiece.getColour() == "White"
				...
				&& !target.isOccupied() && target.getPositionx() == start
				.getPositionx())
	...
	public boolean MovePawn3(CurrentBoard board, Square start, Square target) {
		if ((start.OccupiyingPiece.getColour() == "White"
				&& ((target.getPositionx() == start.getPositionx() + 1) || (target
						.getPositionx() == start.getPositionx() - 1)) && target
				.getPositiony() == start.getPositiony() + 1)
	...
	public boolean CheckPawn2(CurrentBoard board, Square start, Square target,
	...
	public void transform(CurrentBoard board, Square transsquare,
			String desiredpiece) {
		String tempcolour;
		switch (desiredpiece) {
		case "Q":
			...
			board.transformedQ[board.transcount] = new Queen(board, tempcolour,
					transsquare);
		...
		case "N":
			tempcolour = transsquare.OccupiyingPiece.getColour();
			transsquare.OccupiyingPiece.setPosition(board.Squares[0][0]);
			board.transformedN[board.transcount] = new Knight(board,
					tempcolour, transsquare);
		...
		case "B":
			tempcolour = transsquare.OccupiyingPiece.getColour();
			transsquare.OccupiyingPiece.setPosition(board.Squares[0][0]);
			board.transformedB[board.transcount] = new Bishop(board,
					tempcolour, transsquare);
		...
		case "R":
			tempcolour = transsquare.OccupiyingPiece.getColour();
			transsquare.OccupiyingPiece.setPosition(board.Squares[0][0]);
			board.transformedR[board.transcount] = new Rook(board, tempcolour,
					transsquare);
