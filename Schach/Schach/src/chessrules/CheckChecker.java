	...
	public boolean MoveChecks(CurrentBoard board, Square target){
		...
		case "Black":
			...
			case"N":
				...
				n.CheckKnight(board, target, board.pieces.get(0).getPosition());
			case"":
				PawnMovement p = new PawnMovement();
				p.CheckPawn(board, target, board.pieces.get(0).getPosition());
			case"B":
				BishopMovement b = new BishopMovement();
				return b.CheckBishop(board, target, board.pieces.get(0).getPosition());
			}
		case "White":
			switch(target.OccupiyingPiece.getName()){
			case"Q":
			...
			case"N":
				KnightMovement n = new KnightMovement();
				n.CheckKnight(board, target, board.pieces.get(1).getPosition());
			case"":
				PawnMovement p = new PawnMovement();
				p.CheckPawn(board, target, board.pieces.get(1).getPosition());
			case"B":
				BishopMovement b = new BishopMovement();
				return b.CheckBishop(board, target, board.pieces.get(1).getPosition());
			}
		}
